...
Returns the steepness of the [Reward Curve](broken-reference), and [Penalty Curve](broken-reference).
...
Returns the % penalty from unstaking a particular stake. Refer to [Unstake Penalty](broken-reference) for the math. The % Value is in the format of `type(uint16).max`
